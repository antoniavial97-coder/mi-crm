...
type Reminder = { id: string; clientId: string; clientName: string; fechaISO: string; descripcion: string; sourceText: string; notified5d: boolean; notifiedDay: boolean; dismissed: boolean; };
...
const REMINDERS_KEY = "solar-crm:reminders";
    ...
    <div style={{background:isSigned?D.signedBg:D.white,border:`1px solid ${isSigned?D.signedBorder:D.border}`,borderRadius:"14px",padding:"14px",transition:"box-shadow 0.15s"}}
      ...
      <div style={{display:"flex",justifyContent:"space-between",gap:"8px",marginBottom:"8px"}}>
        ...
        <div style={{display:"flex",gap:"4px",flexShrink:0,alignItems:"flex-start"}}>
          <button onClick={()=>setShowDetail(true)} style={{padding:"4px 7px",borderRadius:"7px",border:`1px solid ${D.border}`,background:D.bg,fontSize:"10px",cursor:"pointer",color:D.ink3}} title="Ver historial">📅{meetingCount>0?meetingCount:""}</button>
...
function ActividadRow({act,clientName}:{act:{tipo:string;fecha:string;nota:string;pendiente?:boolean};clientName:string}){
  ...
  useEffect(()=>{
    if(!hasContent||!act.nota)return;
    let cancelled=false;
    fetch("/api/generate-actions",{
      method:"POST",headers:{"Content-Type":"application/json"},
      body:JSON.stringify({
        company:clientName,stage:"actividad",
        comment:`En UNA sola línea corta (máximo 12 palabras), resume la acción principal de este ${tipoLabel.toLowerCase()}: quién hace qué. Ejemplos: "Antonia solicita reunión técnica", "Carlos confirma disponibilidad para el jueves", "Alfonso envía propuesta actualizada con precios". Solo la línea, sin puntos ni explicaciones. Contenido: ${act.nota?.substring(0,600)||""}`,
        transcripts:[]
      })
    }).then(r=>r.json()).then((data:{tasks?:string[]})=>{
      if(!cancelled)setResumen((data.tasks||[])[0]||"");
    }).catch(()=>{});
    return()=>{cancelled=true;};
  },[act.nota]);// eslint-disable-line

  return(
    <div style={{borderRadius:"8px",overflow:"hidden",background:act.pendiente?"#FFFBEB":D.bg,border:act.pendiente?"1px solid #FDE68A":"none"}}>
      <div onClick={()=>hasContent&&setExpanded(e=>!e)} style={{display:"flex",gap:"8px",fontSize:"11px",padding:"6px 10px",alignItems:"center",cursor:hasContent?"pointer":"default"}}>
        <span style={{fontWeight:600,color:act.pendiente?"#d97706":act.tipo.startsWith("✓")?"#16a34a":D.accent,flexShrink:0,minWidth:"65px"}}>
      ...
      {expanded&&hasContent&&(
        <div style={{padding:"6px 10px 10px",borderTop:`1px solid ${D.border}`}}>
          <div style={{fontSize:"12px",color:D.ink2,lineHeight:1.65,background:D.white,borderRadius:"6px",padding:"8px 12px",borderLeft:`3px solid ${D.accent}`,whiteSpace:"pre-wrap"}}>
            {contenido}
          </div>
        </div>
      )}
...
// --- Recordatorios -----------------------------------------------------------
const REMINDER_STORAGE_KEY = REMINDERS_KEY;

function getReminders(): Reminder[] {
  if(typeof window==="undefined")return [];
  try{const r=localStorage.getItem(REMINDER_STORAGE_KEY);return r?JSON.parse(r) as Reminder[]:[];}catch{return [];}
}
...
function Recordatorios({clients,transcripts}:{clients:ClientRecord[];transcripts:TranscriptInfo[]}){
  ...
  // Check daily for notifications (5 days before and on the day)
  useEffect(()=>{
    ...
    function checkReminders(){
      ...
      const in5Str=`${in5.getFullYear()}-${String(in5.getMonth()+1).padStart(2,"0")}-${String(in5.getDate()).padStart(2,"0")}`;
      ...
      for(const r of current){
        ...
        // On the day
        if(r.fechaISO===todayStr&&!r.notifiedDay&&!notifiedRef.current.has(r.id+"-day")){
          notifiedRef.current.add(r.id+"-day");
          new Notification("🔔 Recordatorio hoy",{body:`${r.clientName}: ${r.descripcion}`,icon:"/favicon.ico"});
          r.notifiedDay=true;changed=true;
        }
      }
      if(changed){saveReminders(current);setReminders([...current]);}
    }
    checkReminders();
  ...
  // Analyze all client content for date mentions
  async function analyzeAll(){
    setAnalyzing(true);
    const apiKey=process.env.NEXT_PUBLIC_ANTHROPIC_API_KEY;
    const activeClients=clients.filter(c=>c.stage!=="Perdido"&&c.stage!=="Prospecto Pasivo");
    const newReminders:Reminder[]=[];
    const existing=getReminders().filter(r=>!r.dismissed);

    for(const client of activeClients){
      // Gather all text: meetings, tasks, Diio
      const textParts:string[]=[];
      for(const m of (client.meetings||[]).filter(m=>!m.fromDiio)){
        if(m.notes)textParts.push(`[${m.type} ${m.date}] ${m.notes}`);
      }
      const clientTranscripts=transcripts.filter(t=>t.company.toLowerCase()===client.companyName.toLowerCase());
      for(const t of clientTranscripts){
        if(t.transcript)textParts.push(`[Reunión Diio ${t.date}] ${t.transcript.substring(0,400)}`);
      }
      for(const task of (client.aiTasks||[]).filter(t=>!t.done)){
        textParts.push(`[Tarea] ${task.text}`);
      }
      if(textParts.length===0)continue;
      const fullText=textParts.join("\n");

      try{
        const res=await fetch("/api/extract-reminders",{
          method:"POST",
          headers:{"Content-Type":"application/json"},
          body:JSON.stringify({text:fullText,clientName:client.companyName,clientId:client.id,today:hoy})
        });
        if(!res.ok)continue;
        const data=await res.json() as {reminders?:Array<{fechaISO:string;descripcion:string;sourceText:string}>};
        for(const r of (data.reminders||[])){
          if(!r.fechaISO||r.fechaISO<=hoy)continue;
          const dupKey=`${client.id}|${r.fechaISO}|${r.descripcion.substring(0,30)}`;
          const alreadyExists=existing.some(e=>e.clientId===client.id&&e.fechaISO===r.fechaISO&&e.descripcion.substring(0,30)===r.descripcion.substring(0,30));
          if(!alreadyExists){
            newReminders.push({id:newId(),clientId:client.id,clientName:client.companyName,fechaISO:r.fechaISO,descripcion:r.descripcion,sourceText:r.sourceText,notified5d:false,notifiedDay:false,dismissed:false});
          }
        }
      }catch{}
    }
    const merged=[...existing,...newReminders].sort((a,b)=>a.fechaISO.localeCompare(b.fechaISO));
    setReminders(merged);
    saveReminders(merged);
    if(typeof window!=="undefined")localStorage.setItem("solar-crm:reminders-analyzed",hoy);
    setLastAnalyzed(hoy);
    setAnalyzing(false);
  }
    ...
    <div style={{background:D.white,border:`1px solid ${D.border}`,borderRadius:"16px",overflow:"hidden",boxShadow:D.shadow}}>
      <button onClick={()=>setOpen(o=>!o)} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 18px",background:"none",border:"none",cursor:"pointer"}}>
        <div style={{display:"flex",alignItems:"center",gap:"12px"}}>
          <div style={{width:"36px",height:"36px",borderRadius:"10px",background:active.some(r=>r.fechaISO<=hoy5ISO)?"linear-gradient(135deg,#F59E0B,#EF4444)":"#F3F4F6",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"16px",flexShrink:0}}>📌</div>
          <div style={{textAlign:"left"}}>
            <div style={{fontSize:"13px",fontWeight:600,color:D.ink,display:"flex",alignItems:"center",gap:"8px"}}>
              Recordatorios detectados
              {active.length>0&&<span style={{fontSize:"10px",background:active.some(r=>r.fechaISO<=hoy5ISO)?"#FEF3C7":"#F3F4F6",color:active.some(r=>r.fechaISO<=hoy5ISO)?"#B45309":D.ink3,padding:"2px 8px",borderRadius:"10px",fontWeight:600}}>{active.length} pendiente{active.length>1?"s":""}</span>}
            </div>
            <div style={{fontSize:"11px",color:D.ink3,marginTop:"2px"}}>{lastAnalyzed?`Último análisis: ${formatDateShort(lastAnalyzed)}`:"Sin analizar aún"}</div>
          </div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
          <button onClick={e=>{e.stopPropagation();analyzeAll();}} disabled={analyzing} style={{padding:"5px 12px",borderRadius:"8px",border:`1px solid ${D.border}`,background:"#F9FAFB",fontSize:"11px",cursor:analyzing?"not-allowed":"pointer",color:D.ink2,fontWeight:500}}>
            {analyzing?"⏳ Analizando...":"✦ Analizar"}
          </button>
          <span style={{fontSize:"12px",color:D.ink3}}>{open?"▲":"▼"}</span>
        </div>
      </button>
      {open&&(
        <div style={{borderTop:`1px solid ${D.border}`,padding:"12px 16px",display:"flex",flexDirection:"column",gap:"8px"}}>
          {active.length===0?(
            <div style={{textAlign:"center",padding:"16px",color:D.ink3,fontSize:"12px"}}>
              {lastAnalyzed?"No hay recordatorios detectados. Apretá ✦ Analizar para buscar fechas en correos, reuniones y tareas.":"Apretá ✦ Analizar para detectar fechas y compromisos mencionados en correos, reuniones y tareas."}
            </div>
          ):active.map(r=>{
            const esProximo=r.fechaISO<=hoy5ISO;
            const esHoy=r.fechaISO===hoy;
            return(
              <div key={r.id} style={{display:"flex",alignItems:"flex-start",gap:"10px",padding:"10px 12px",borderRadius:"10px",background:esHoy?"#FEF3C7":esProximo?"#FFF7ED":"#F9FAFB",border:`1px solid ${esHoy?"#FCD34D":esProximo?"#FED7AA":D.border}`}}>
                <div style={{flexShrink:0,minWidth:"52px",textAlign:"center"}}>
                  <div style={{fontSize:"16px",fontWeight:700,color:esHoy?"#92400E":esProximo?"#C2410C":D.ink,lineHeight:1}}>{r.fechaISO.slice(8)}</div>
                  <div style={{fontSize:"9px",color:D.ink3,textTransform:"uppercase"}}>{["ene","feb","mar","abr","may","jun","jul","ago","sep","oct","nov","dic"][parseInt(r.fechaISO.slice(5,7))-1]} {r.fechaISO.slice(0,4)}</div>
                </div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:"11px",fontWeight:600,color:D.ink}}>{r.clientName}</div>
                  <div style={{fontSize:"12px",color:D.ink2,marginTop:"2px"}}>{r.descripcion}</div>
                  {r.sourceText&&<div style={{fontSize:"10px",color:D.ink3,marginTop:"3px",fontStyle:"italic",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{r.sourceText.substring(0,80)}</div>}
                </div>
                <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:"4px",flexShrink:0}}>
                  {esHoy&&<span style={{fontSize:"10px",fontWeight:700,color:"#92400E",background:"#FCD34D",padding:"2px 7px",borderRadius:"10px"}}>HOY</span>}
                  {esProximo&&!esHoy&&<span style={{fontSize:"10px",fontWeight:600,color:"#C2410C",background:"#FFEDD5",padding:"2px 7px",borderRadius:"10px"}}>Próximo</span>}
                  <button onClick={()=>dismiss(r.id)} style={{padding:"3px 8px",borderRadius:"6px",border:`1px solid ${D.border}`,background:D.white,fontSize:"10px",cursor:"pointer",color:D.ink3}}>✕ Descartar</button>
                </div>
              </div>
            );
          })}
