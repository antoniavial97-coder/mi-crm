...
// Contacts sheet: gid=1 (segunda hoja). Columnas esperadas: Empresa, Contacto, Email, Teléfono
const CONTACTS_CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQx9xTTA1PLUjIbfcEQa4J8s-vazmF_VGGgDQwP4CEoPI3Dy1oimVkRg3YLeFRvyP04IvY5fgMVci2t/pub?gid=1&single=true&output=csv";
...
// P1 display order (most important first)
...
function parseContactsCSV(csv:string):ContactInfo[]{
  ...
  const idx={company:col(["empresa","company"]),name:col(["contacto","nombre","contact","name"]),email:col(["email","correo","mail"]),phone:col(["teléfono","telefono","phone","tel","cel"])};
  return lines.slice(hLine+1).map(line=>{
    const cols=parseCSVLine(line);
    const get=(i:number)=>(i>=0?(cols[i]??"").trim():"");
  ...
  function toggleTask(clientId:string,taskId:string){
    setLocalTasks(prev=>{
      const tasks=(prev[clientId]||[]).map(t=>{
        ...
        const done=!t.done;
        // Si se marca como hecho y el texto sugiere envío de correo/mensaje, crear follow-up
...
function MonthlyChart({clients}:{clients:ClientRecord[]}){
  const data=useMemo(()=>{
    // Generate months May 2026 to May 2027
    const months:string[]=[];
  ...
  const maxAcc=Math.max(...data.map(d=>d.acc),0.01);
  return(
    <div style={{background:D.white,border:`1px solid ${D.border}`,borderRadius:"16px",padding:"1.5rem",flex:1}}>
      ...
      {/* Client list for months with closings */}
...
// ─── Tab Views ────────────────────────────────────────────────────────────────
function Pipeline1Tab({clients,contacts,onEdit,onDelete,onUpdateTasks}:{clients:ClientRecord[];contacts:ContactInfo[];onEdit:(id:string)=>void;onDelete:(id:string)=>void;onUpdateTasks:(id:string,tasks:ClientTask[])=>void}){
  ...
  const metrics=useMemo(()=>{
    ...
    return{total:p1.length,active:active.length,signed:signed.length,mwpActive:active.reduce((s,c)=>s+(c.mwp||0),0),mwpSigned:signed.reduce((s,c)=>s+(c.mwp||0),0)};
  },[p1]);
  return(
            ...
            <div key={sub} style={{background:isSigned?D.signedBg:D.white,border:`1px solid ${isSigned?D.signedBorder:D.border}`,borderRadius:"16px",padding:"1.25rem",borderLeft:`3px solid ${isSigned?"#22c55e":D.accent}`}}>
              ...
              {items.length?(
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:"10px"}}>
                  {items.map(c=><ClientCard key={c.id} client={c} contacts={contacts} onEdit={onEdit} onDelete={onDelete}/>)}
                </div>
              ):(
                <div style={{borderRadius:"10px",border:`1px dashed ${D.border}`,padding:"1rem",fontSize:"12px",color:D.ink3,textAlign:"center"}}>Sin clientes</div>
              )}
            </div>
          );
        })}
...
function Pipeline2Tab({clients,contacts,onEdit,onDelete,onUpdateTasks}:{clients:ClientRecord[];contacts:ContactInfo[];onEdit:(id:string)=>void;onDelete:(id:string)=>void;onUpdateTasks:(id:string,tasks:ClientTask[])=>void}){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:"1.5rem"}}>
      ...
      <AIPendientesPanel clients={p2} onUpdateTasks={onUpdateTasks}/>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:"10px"}}>
        {p2.length?p2.map(c=><ClientCard key={c.id} client={c} contacts={contacts} onEdit={onEdit} onDelete={onDelete}/>):(
          <div style={{gridColumn:"1/-1",borderRadius:"12px",border:`1px dashed ${D.border}`,padding:"2rem",textAlign:"center",fontSize:"13px",color:D.ink3}}>Sin clientes en Pipeline P2</div>
        )}
      </div>
    </div>
  );
...
function ProspectosTab({clients,contacts,onEdit,onDelete,onUpdateTasks}:{clients:ClientRecord[];contacts:ContactInfo[];onEdit:(id:string)=>void;onDelete:(id:string)=>void;onUpdateTasks:(id:string,tasks:ClientTask[])=>void}){
  const activos=clients.filter(c=>c.stage==="Prospecto Activo");
  const pasivos=clients.filter(c=>c.stage==="Prospecto Pasivo");
  return(
    <div style={{display:"flex",flexDirection:"column",gap:"1.5rem"}}>
      <AIPendientesPanel clients={[...activos,...pasivos]} onUpdateTasks={onUpdateTasks}/>
      {[{label:"Prospectos Activos",items:activos},{label:"Prospectos Pasivos",items:pasivos}].map(({label,items})=>(
        <div key={label}>
          <div style={{fontSize:"14px",fontWeight:600,color:D.ink,marginBottom:"10px",display:"flex",alignItems:"center",gap:"8px"}}>
            <div style={{width:"3px",height:"14px",borderRadius:"2px",background:D.accentY}}/>
            {label} <span style={{fontSize:"12px",color:D.ink3,fontWeight:400}}>· {items.length} clientes</span>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:"10px"}}>
            {items.length?items.map(c=><ClientCard key={c.id} client={c} contacts={contacts} onEdit={onEdit} onDelete={onDelete}/>):(
              <div style={{gridColumn:"1/-1",borderRadius:"12px",border:`1px dashed ${D.border}`,padding:"1.5rem",textAlign:"center",fontSize:"12px",color:D.ink3}}>Sin clientes</div>
            )}
          </div>
        </div>
      ))}
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"14px",marginBottom:"14px"}}>
        ...
        <div style={{gridColumn:"1/-1"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"5px"}}>
            ...
            <button disabled={extractTasksLoading} onClick={onExtract} style={{padding:"4px 12px",borderRadius:"8px",border:"1px solid #DDD6FE",background:"#F5F3FF",fontSize:"11px",cursor:"pointer",color:"#7C3AED"}}>
              {extractTasksLoading?"Extrayendo…":"Extraer tareas IA"}
            </button>
          </div>
  ...
  return(
    <div style={{minHeight:"100dvh",background:D.bg}}>
      ...
      <div style={{maxWidth:"1280px",margin:"0 auto",padding:"2rem"}}>
        ...
        {/* Pipeline 1 Tab */}
        {activeTab==="pipeline1"&&(
          <Pipeline1Tab clients={clients} contacts={contacts} onEdit={openEdit} onDelete={removeClient} onUpdateTasks={updateClientTasks}/>
        )}

        {/* Pipeline 2 Tab */}
        {activeTab==="pipeline2"&&(
          <Pipeline2Tab clients={clients} contacts={contacts} onEdit={openEdit} onDelete={removeClient} onUpdateTasks={updateClientTasks}/>
        )}

        {/* Prospectos Tab */}
        {activeTab==="prospectos"&&(
          <ProspectosTab clients={clients} contacts={contacts} onEdit={openEdit} onDelete={removeClient} onUpdateTasks={updateClientTasks}/>
        )}
      </div>
